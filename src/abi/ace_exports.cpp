...
UNSIGNED32 AdsOpenTable(ADSHANDLE  hConnect,
    ...
    *phTable = gh;
    ...
    // (or `<base>.dat` when ERP keeps ADT data in .DAT + .ADI per ARC-CAJA)
    // auto-binds `<base>.adi` if it exists.
    {
        std::string extl = tp.extension().string();
        for (auto& ch : extl) ch = static_cast<char>(std::tolower(static_cast<unsigned char>(ch)));
        if (extl == ".adt" || extl == ".dat") {
